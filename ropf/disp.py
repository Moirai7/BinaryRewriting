...
class Displacement:
    ...
    def _decide_displacement(self, disp_rate, intended_gads, seed, rand=False, DEBUG=False):
        '''
        Main strategy
        ...
        cnt = 0
        if gad_cnt < int(seed):
            print('Wrong, the seed is larger than gad_cnt', seed, gad_cnt)
            sys.exit(1)
        print(gad_cnt, seed)
        for gad_start in gad_starts[0:int(seed)]:
            if not DEBUG:
                bar.update(cnt+1)
    ...
    def auto_disp(self, input_file, output_file, seed, rand=False, DEBUG=False):
        """
        ...
        self._show_steps(5)
        s = time.time()
        self._decide_displacement(1, gc.intended, seed, rand, DEBUG)
