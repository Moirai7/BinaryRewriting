...
def displace(input_file, seed, name, rand=False, DEBUG=False):
    print(input_file, seed, name)
    p = os.path.abspath(input_file)
    b = os.path.basename(p)
    output_file = p + "_" + name
    print(output_file)
    dp = disp.Displacement(p)
    dp.auto_disp(input_file, output_file, seed, rand, DEBUG)

def randomize(input_file, seed, name):
    ...
    inp.patch(global_diffs, name, False)
...
if __name__=="__main__":
    ...
    parser.add_option("-i", "--iter_name", dest="iter_name",
