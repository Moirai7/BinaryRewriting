...
def call_rand(input_file, seed, iters):
...
def sai():
    ...
    for i in range(start,start+100):
        f = ori[i]
        fn = f.split('\\')[-1]
        feat_loc = 'data/tmp2/'+fn
        os.makedirs('data/tmp2/', exist_ok=True)
        if fn in checked or os.path.exists(feat_loc):
            continue
        print(i, 'total '+str(total)+' enum '+str(enum)+" " + f+'\n')
        ...
        while (iterations < 200): 
            ...
            for sample in glob.glob("data/tmp2/*.diff"):
                os.remove(sample)
            if os.path.exists(f2+".dmp.bz2"):
                os.remove(f2+".dmp.bz2")
            if os.path.exists("data/tmp2/"+'.'.join(fn.split('.')[0:-1])+".idb"):
                os.remove("data/tmp2/"+'.'.join(fn.split('.')[0:-1])+".idb")
            call_ida(f2)
            rest = random.randint(1,4)
            call_rand(f2, rest, iterations)
            ...
            if np.argmax(logits,-1)[0] == config.labels['ben']:
                ...
                check = 1
                copyfile(feat_loc, 'data/succ/'+feat_loc.split('/')[-1])
                break
